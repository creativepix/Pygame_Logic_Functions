...
from source_code.constants import TRAINING_INSTRUCTIONS, \
    ...
    BLOCKS_NAME_COLOR, TRAINING_UPPER_TEXT_RECT, \
    TRAINING_UPPER_TEXT_MAX_SYMBOLS, TRAININ_TEXT_LINES_INDENT, \
    TRAINING_ARROW_SIZE, SCORE_GAME_RECT
...
class TrainingWindow(PlayWindow):
    ...
    def mouse_down(self, mouse_button: int) -> None:
        ...
        if mouse_button == 1 and self.stage in \
                [0, 1, 2, 3, 4, 8, 10, 14, 15, 16]:
            self.stage += 1
            if self.stage == 17:
                to_main_menu()
            super().mouse_down(mouse_button)
        elif mouse_button == 3 and self.stage == 11:
            super().mouse_down(mouse_button)
    ...
    def tick(self, screen: pygame.Surface) -> None:
        ...
        if self.stage in [3, 9]:
            ...
            rect.x += rect.w
            rotating = 180
        if self.stage in [3, 4, 5, 9, 11, 13, 14, 15] and rect is not None:
